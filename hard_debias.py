    ...
    for equal_set in equality_sets:
        if equal_set[0] in w2id and equal_set[1] in w2id:
            ...
            for word in equal_set:
                ...
                # new_embed = v + np.sqrt(1 - (np.linalg.norm(v)) ** 2) * ((word_biased - mean_biased) / utils.unit_vec(word_biased - mean_biased))
                # slightly modified because in our case np.linalg.norm(v) is always 1 and np.sqrt(0) is not possible!
                new_embed = v * ((word_biased - mean_biased) / utils.unit_vec(word_biased - mean_biased))
