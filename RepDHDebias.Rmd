...
@wang_2020 applied this evaluation method to all the different embedding baselines and their Double-Hard debiased embedding as well. However, we found the code provided by the authors to be incomplete. It was missing major parts <!-- maybe not state what exactly was missing, but rather define it to as "major parts" or something like that?--> and due to time constraints we decided not to implement this method. <!--KK: letzten Satz würde ich auch rauslassen. Uns ging es ja ums re-implementen, da ist es vollkommen egal, ob wir glauben, dass man das schon mit dem word analogy task gezeigt hat (und im Übrigen zeigen die beiden Tests ja auch völlig unterschiedliche Eigenschaften des semantic space)-->
