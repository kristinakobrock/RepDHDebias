...
weat_results <- read.csv('code/evaluation_results/results_weat.csv')
nm_results <- read.csv('code/evaluation_results/results_nm.csv')
msr_results <- read.csv('code/evaluation_results/results_msr.csv')
gg_results <- read.csv('code/evaluation_results/results_gg.csv')
```
...
```{r tsne plot}
knitr::include_graphics("code/evaluation_results/results_tsne.png")
```
...
- Meta (Krissi):
  ...
  - uploaded notebook only exemplary
-->
The code provided by @wang_2020 was incomplete at several points which hindered both our implementation of the Double-Hard Debias method and the Evaluation. For example the authors' notebook `GloVe_Debias` only showcases the application of Double-Hard Debias but does not provide the actual embedding that was used by @wang_2020 in the evaluation as a result. This can be seen as several pre-processing steps are applied in `GloVe_Debias` such as the restriction of the vocabulary to 50.000 words or the removal of words containing digits. These steps go unmentioned in @wang_2020's documentation and were not applied in the missing code that actually resulted in the Double-Hard debiased embedding as this is not restricted and of the same length as the original embedding. We decided to adopt some of the pre-processing steps oriented on their github repository for computational reasons.
We also noted that from the code provided by @wang_2020 it was not apparent whether they used the word pairs from `equalize_pairs` or those from the combination of `male_word_file` and `female_word_file` for the equalization step of the Hard debias method. We were able to report that this does not change the results, however it is another example of ill documentation. 
At many points throughout the evaluations, most noticeably the analysis of retaining word semantics, we were confronted with the `web` module used by @wang_2020. However the authors did not provide this module, neither did they link or even specify where the datasets and functions included could be found. This resulted in the need of self-implementing some functions as well as fundiing the datasets they used and pre-processing them such that they were usable with @wang_2020's functions. Due to time constraints we were not able to cover everything included in this module which lead to us not implementing the Concept Categorization for our evaluation.
...
We obtained results for three parts of the evaluation, namely WEAT, the Neighborhood Metric and the Word Analogy Task. For the Neighborhood Metric we obtained results that closely resemble those of @wang_2020. Similarly we can report results comparable to those of the authors for the Word Analogy Task. However, after applying WEAT we obtained values that differ from those of @wang_2020 for two of three word sets. The closely matching values we obtained for *Caree & Family* suggest a good replication of the authors' results. The differences for the other word sets do not necessarily change the interpretation but are still concerning.

Preprocessing is one of the main aspects not typically reported in scientific NLP papers that can cause experimental variation identified by @fokkens_2013. In line with this, the paper by @wang_2020 did not specify how the relevant datasets were preprocessed. This could be one of the reasons why our results deviate from the results obtained by the original authors. While we were able to reproduce the main conclusions and findings of the paper, we were not able to reproduce concrete values. As @mieskes_2019 point out: "Experiments in replication fail more often than not." Whereas the replication attempt described in this report was not a complete failure, many difficulties were identified during the process of replication due to incomplete documentation of the original study and lack of provided code. The main reasons for this appear to be common in the field: incomplete documentation, missing files and non-working links [see @mieskes_2017; @collberg_2015].

<!--MK: content-->
When working as closely with the paper as a reproduction study necessitates, we noticed one of the authors' claims not to be sufficiently proven. @wang_2020 claim that the success of their algorithm can be explained by the mitigation of frequency information in embeddings before debiasing, but there seems to be no clear-cut proof for that. It might be a reasonable belief, as frequency has to be shown to be a prominent characteristic of trained embeddings [@gong_2018] and represented to a significant amount in multiple of the top ptincipal components [@mu_2018] of those. Nevertheless, one cannot simply pick out one of those principal components and claim them to represent any  unique feature, just as @mu_2018 present their method and its result without making any definite claims about general relations to word frequency information. When making use of exactly those finding though, @wang_2020 twist one observation of frequency features in a multitude of top components to a hard and universal truth.
...
Furthermore, @wang_2020 do not propose a specific PC to remove, but base their choice on which projection will lead to most favorable results. Meaning, always removing that direction that will optimize subsequent performance of Hard Debias. This will lead to varying directions being choosen for different embeddings, suggesting the largest PC on one, the fifth or sixth PC on a different set of embeddings. Ultimately, by claiming that Doublle-Hard Debias removes frequency information, the authors'  also state that these must be embedded in potentially any of the principal components, especially as their pseudocode consideres all possible PCs as candidates.
